...
from backend_core.modules.tenants.telephony import PlatformTelephonyService
from backend_core.platform.auth import require_admin
from backend_core.platform.database import DatabaseSession

router = APIRouter(prefix="/admin/v1/tenants", tags=["admin:tenants"], dependencies=[Depends(require_admin)])
internal_router = APIRouter(prefix="/internal/v1", tags=["internal:tenants"])
platform_router = APIRouter(prefix="/admin/v1/platform/prompts", tags=["admin:platform-prompts"], dependencies=[Depends(require_admin)])
telephony_platform_router = APIRouter(prefix="/admin/v1/platform/telephony", tags=["admin:platform-telephony"], dependencies=[Depends(require_admin)])
...
async def create_tenant(data: CreateTenantRequest, service: TenantServiceDependency) -> TenantResponse:
    try:
...
async def get_tenant_by_slug(slug: Slug, service: TenantServiceDependency) -> TenantResponse:
    try:
...
async def get_tenant(tenant_id: UUID, service: TenantServiceDependency) -> TenantResponse:
    try:
...
async def list_tenants(
    ...
    return [TenantResponse.model_validate(item) for item in await service.list(offset=offset, limit=limit)]


def platform_telephony_service(session: DatabaseSession, request: Request) -> PlatformTelephonyService:
    return PlatformTelephonyService(
        TelephonyRepository(session), request.app.state.livekit, request.app.state.settings,
        request.app.state.outbox_tracer, request.app.state.core_metrics,
    )


PlatformTelephonyServiceDependency = Annotated[PlatformTelephonyService, Depends(platform_telephony_service)]


@telephony_platform_router.get("", response_model=PlatformTelephonyResponse)
async def show_platform_telephony(service: PlatformTelephonyServiceDependency) -> PlatformTelephonyResponse:
    return await service.show()


@telephony_platform_router.post("/reconcile", response_model=PlatformTelephonyResponse)
async def reconcile_platform_telephony(service: PlatformTelephonyServiceDependency) -> PlatformTelephonyResponse:
