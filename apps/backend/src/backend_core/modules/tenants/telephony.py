...
from backend_core.modules.tenants.schemas import PlatformTelephonyResponse
    ...
    async def reconcile(self) -> PlatformTelephonyResponse:
        ...
        if not self._settings.sip_provider_address:
            return await self._set_failure(
                state, provisionings, TelephonyProvisioningStatus.DEGRADED,
                "SIP provider connection is not configured", started,
            )
        numbers = sorted(
        ...
        try:
            with domain_span(self._tracer, "telephony.reconcile"):
                inbound, outbound, dispatch = await self._livekit.reconcile_shared_sip(
                    ...
                    provider_username=(
                        ...
                        if self._settings.sip_provider_username else None
                    ),
                    provider_password=(
                        self._settings.sip_provider_password.get_secret_value()
                        if self._settings.sip_provider_password else None
                    ),
            ...
            state.inbound_trunk_id, state.outbound_trunk_id, state.dispatch_rule_id = inbound, outbound, dispatch
            ...
            return await self._set_failure(
                state, provisionings, TelephonyProvisioningStatus.ERROR,
                f"LiveKit reconciliation failed ({type(error).__name__})", started,
            )
        return await self.show()

    async def _set_failure(self, state, items, status, message: str, started: float) -> PlatformTelephonyResponse:
...
class PlatformTelephonyReconciler:
    def __init__(self, database: Database, livekit: LiveKitAdapter, settings: Settings, tracer: Tracer | None = None, metrics: CoreMetrics | None = None) -> None:
        ...
        while True:
            try:
                async with self._database.transaction() as session:
                    ...
                    if (
                        platform.provisioning_status is TelephonyProvisioningStatus.PENDING
                        or any(item.status == TelephonyProvisioningStatus.PENDING.value for item in provisionings)
                    ):
                        await PlatformTelephonyService(repository, self._livekit, self._settings, self._tracer, self._metrics).reconcile()
            except Exception:
