...
def profile(**changes: object) -> dict[str, object]:
    return {
        ...
        "input_constraints": [{"kind": "date_range", "start": "stay.check_in", "end": "stay.check_out"}],
        "execution": {
            ...
            "request": {"codec": "json", "mapping": {"guest": {"$expr": "request.phone"}}},
...
def test_tenant_capabilities_preserve_http_mapping_and_typed_ref() -> None:
    value = TenantCapabilitiesConfig.model_validate({"capabilities": {"reservation.create": profile()}})
...
def test_capability_registry_is_tenant_only() -> None:
    ...
    assert registry.resolve(ComponentAddress(ComponentKind("capabilities.tenant"), TenantScope("t")))
    with pytest.raises(ScopeNotAllowed):
        registry.resolve(ComponentAddress(ComponentKind("capabilities.tenant"), PlatformScope()))


def test_tool_name_collision_and_binding_normalization_are_deterministic() -> None:
    assert derive_tool_name("x." + "a" * 80).endswith("_" + derive_tool_name("x." + "a" * 80).rsplit("_", 1)[1])
    assert normalize_canonical_input({"phone": "00 421 912 123 456", "custom": "  unchanged  "}, {"phone": "guest.phone"}) == {"phone": "+421912123456", "custom": "  unchanged  "}
    with pytest.raises(ValidationError, match="duplicate derived tool name"):
        TenantCapabilitiesConfig.model_validate({"capabilities": {"a.b": profile(), "a_b": profile()}})
...
def test_unsupported_policy_and_invalid_date_binding_are_rejected() -> None:
    with pytest.raises(InvalidComponentValue, match="Availability proof"):
        TenantCapabilitiesConfig.model_validate({"capabilities": {"reservation.create": profile(business_policy={"requires_availability_proof": True})}})
    broken = profile(input_constraints=[{"kind": "date_range", "start": "guest.phone", "end": "stay.check_out"}])
    with pytest.raises(InvalidComponentValue, match="date range fields"):
        TenantCapabilitiesConfig.model_validate({"capabilities": {"reservation.create": broken}})
